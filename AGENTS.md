# Project: HincyRay v0.5 (crate `xray-vpn-test`)

Rust crate shipping two binaries: the `hincyray` router daemon (Keenetic Giga KN-1012 / Entware aarch64, safe SOCKS-only MVP by default, with an opt-in WiFi VPN segment added in v0.1.1, WiFi-only traffic split added in v0.3, tun2socks-based WiFi VPN routing replacing TPROXY in v0.4, and HWID/protocol/port/DNS/geo features in v0.5) and the `xray-vpn-test` desktop diagnostics app (macOS, feature `desktop`). Both reuse `src/profiles.rs`, `src/scoring.rs`, and `src/xray_config.rs`.
...
* `src/hincyray.rs` - HincyRay router daemon: sync `TcpListener` HTTP API, state persistence, `CoreManager` for Xray process lifecycle (with `XRAY_LOCATION_ASSET` env), `TunManager` for tun2socks process lifecycle, WiFi-only split routing via tun2socks, per-server QUIC toggle, port routing, DNS anti-leak, HWID fingerprint, TUN controls in web UI.
